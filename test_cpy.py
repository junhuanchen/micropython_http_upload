...
class MicroWebCli :
    ...
    def GETRequest(url, queryParams=None, auth=None, connTimeoutSec=5, socks5Addr=None) :
    ...
    def POSTRequest(url, formData={}, auth=None, connTimeoutSec=5, socks5Addr=None) :
    ...
    def JSONRequest(url, o=None, auth=None, connTimeoutSec=5, socks5Addr=None) :
        c = MicroWebCli( url,
    ...
    def FileRequest(url, filepath, progressCallback=None, auth=None, connTimeoutSec=5, socks5Addr=None) :
        c = MicroWebCli(url, auth=auth, connTimeoutSec=connTimeoutSec, socks5Addr=socks5Addr)
        c.OpenRequest()
        r = c.GetResponse()
        if r.IsSuccess() :
    ...
    def __init__(self, url='', method='GET', auth=None, connTimeoutSec=5, socks5Addr=None) :
        self.URL            = url
        self.Method         = method
        self.Auth           = auth
    ...
    def OpenRequest( self,
        ...
        if self.Proto == 'https' :
            ...
            try :
                try :
                    cli = ussl.wrap_socket(cli, timeout=self.ConnTimeoutSec)
                except TypeError :
                    cli = ussl.wrap_socket(cli)
            except Exception as ex :
                cli.close()
    ...
    class _response :
        ...
        def _parseFirstLine(self) :
            self._httpVer, code, self._msg = self._socket.makefile().readline() \
                                                         .strip()    \
                                                         .split(' ', 2)
            self._code = int(code)
        ...
        def _parseHeader(self) :
            while True :
                elements = self._socket.makefile().readline() \
                                       .strip()    \
                                       .split(':', 1)
                if len(elements) == 2 :
...
class MultiPartForm(object):
    ...
    def put_binary(self, binary_write):
        ...
        for fieldname, filename, content_type, fileSize in self.files:
            ...
            binary_write(b'\r\n'.join(block))
            binary_write(b'\r\n')
            fileObject = open(fieldname + filename, 'rb')
            
            while True:
                ch = fileObject.read(1024)
                if not ch:
                    break
                binary_write(ch)
            fileObject.close()
...
filename = 'hfshttp.zip'
filepath = './'
fileObject = open(filepath + filename, 'rb')
fileSize = os.stat(filepath + filename)[6]
print(filepath + filename, fileObject, fileSize)

# wCli = MicroWebCli('http://192.168.43.66:8080', 'POST')
